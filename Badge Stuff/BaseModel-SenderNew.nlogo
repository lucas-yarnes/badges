...
globals [ num-sick group-id run-number delay url ]

breed [ androids android ]

turtles-own [ infected? uuid ]
...
to setup
  ...
  set group-id (word "" (random 1e9))
  clear-output
  output-print group-id

  set url "http://rendupo.com:8000/badgerstate"

  create-androids num-androids [
    setxy random-pxcor random-pycor
    set color (one-of [5 35 45 55 65 75 85 95 105 115 135])
    set heading 90 * random 4
    set infected? false
    set uuid (first web:make-request (word url "/join/" group-id) "POST" [] [])
    let data (word xcor "%20" ycor "%20" color)
    __ignore (web:make-request (word url "/data/" group-id "/" uuid "/" data) "POST" [] [])
  ]
...
to get-sick
  if not infected? [
    ...
    __ignore web:make-request (word url "/signal/" group-id "/" uuid "/infected") "POST" [] []
  ]
end

to disinfect
  if any? androids with [infected?] [
    ask one-of androids with [infected?] [
      set infected? false
      set shape "android"
      __ignore web:make-request (word url "/signal/" group-id "/" uuid "/cured") "POST" [] []
    ]
  ]
end
...
to go
  ...
  ask androids [
    rt (random 4) * 90
    fd 1
    let coords (word xcor "%20" ycor)
    __ignore web:make-request (word url "/data/" group-id "/" uuid "/" coords) "POST" [] []
  ]

  ask turtles with [infected?] [
    ask other turtles-here with [not infected? and (random 100 < infection-chance)] [ get-sick ]
  ]

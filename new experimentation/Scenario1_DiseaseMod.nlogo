...
to setup
  ...
  ifelse use-percent
  [
    set num-initial ceiling (percent-initial-infected / 100.0 * count badges)
...
to check
  ...
  [
    ask badge1 [ create-link-with badge2 [ set color red set thickness .3 ] set color blue set infected? true]
    ask badge2 [ set color blue set infected? true]
  ]
  [
    ask badge1 [ create-link-with badge2 [ set color gray set thickness .2 ]]
  ]
end
...
ask one-of badges with [module-id = turtle-one] [ user-message (word \"There are \" count nw:turtles-in-radius radius \" badges in network radius \" radius \" of badge #\" turtle-one) ]
NIL
1
T
OBSERVER
...
749
636
How Many Infected At First?
count badges with [ first-infected? = true ]
17
1
11
...
[module-id] of badges with [first-infected? = true]
17
1
11
...
868
50
Layout around Most Connected
layout-radial turtles links one-of badges with [ count link-neighbors = max [count link-neighbors] of badges ]
NIL
1
T
OBSERVER
